...
func TestLevenshteinAutomaton(t *testing.T) {
	...
	words := []string{"car", "card", "care", "careful", "cat", "dog", "dogs"} // Already sorted
	...
	expected := []string{"car", "card", "care", "cat"}
	if len(results) != len(expected) {
		t.Errorf("Expected %d results, got %d. Results: %v", len(expected), len(results), results)
	}
